...
#include "../include/LocalYuvToH264.h"
LocalYuvToH264::LocalYuvToH264() {
...
void LocalYuvToH264::checkYuvInfo(char *localYuvPath) {
    formatCtx = avformat_alloc_context();
    int ret = avformat_open_input(&formatCtx, localYuvPath, nullptr, nullptr);
    if (ret < 0) {
        std::cout << "avformat_open_input fail " << ret << std::endl;
        return;
    }
    avformat_find_stream_info(formatCtx, nullptr);
    av_dump_format(formatCtx, AVMEDIA_TYPE_VIDEO, localYuvPath, 0);

    avformat_close_input(&formatCtx);
}

int LocalYuvToH264::init(char *localYuvPath, int height, int width, char *saveH264Path) {
